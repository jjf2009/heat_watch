...
export default function ExportPDF({ data }: Props) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      pdf.setFont("helvetica", "italic");
      pdf.setTextColor(120, 120, 120);
      pdf.text("Generated by Groq LLM (llama-3.1-8b-instant) based on real-time UHI metrics.", margin, y);
      y += 7;
      ...
      if (aiRecs.length === 0) {
        ...
        aiRecs.forEach((rec, i) => {
          ...
          const boxH = Math.max(22, 14 + detailH);
      ...
      y += 5; // Extra padding before the next section
      // ─── ML Climatology (ONNX) ─────────────────────────────────────────

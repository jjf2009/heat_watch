...
export default function PeakHourPanel({ hourlyPattern, peakHour, ruralBaseline }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-6">
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="font-bold text-lg">⏱️ Peak UHI Hours</h3>
        ...
        <div className="bg-red-50 border border-red-200 rounded-xl px-3 py-2 text-center">
          ...
          <p className="text-xs text-red-400">+{peakHour.uhiDelta.toFixed(1)}°C</p>
        </div>
      </div>
            ...
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              <div
                ...
                style={{
                  ...
                  background: isPeak ? "#ef4444" : h.uhiDelta > 3 ? "#f97316" : "#fbbf24",
      ...
      <div className="mt-4 p-3 bg-amber-50 rounded-xl">
        <p className="text-sm text-amber-800">
          <strong>Advisory:</strong> UHI peaks at {peakHour.time} reaching +{peakHour.uhiDelta.toFixed(1)}°C
          above rural baseline of {ruralBaseline.toFixed(1)}°C.
          Avoid outdoor activity between {peakHour.time} and the following 2 hours.
        </p>
      </div>
    </div>
  );
